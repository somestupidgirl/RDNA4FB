...
| `rdna4-dmubver=1` | Read-only fingerprint of the GOP-loaded DMUB firmware (full `DMCUB_SCRATCH` bank + boot/enable state). Compare the values against Debian's `dmesg \| grep -i dmub` version to tell whether the GOP's DMUB speaks the mainline VBIOS command dialect (subtypes 0/1/2, safe to replay) or its own (the GOP's DP bring-up used 6/10/12/16). |
